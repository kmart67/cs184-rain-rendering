...
#    normal = plane.normal
    ...
    for index in range(len(droplets)):
        ...
        for i in range(frames):
            ...
            for v in bm.verts:
                if i != 0:
                    _, co_kf, _, vel, _ = keyframe_collisions[i - 1][v]
                else:
                    co_kf = v.co
                ...
                # num_neighbors = len(v.link_edges) + 1
                #
                # # Create identity matrix.
                # identity = np.identity(num_neighbors)
                #
                # # Create lumped mass matrix.
                # lumped_mass_matrix = np.identity(num_neighbors)
                #
                # # Create Laplacian-Beltrami matrix.
                # lap_bel_matrix = np.zeros((num_neighbors, num_neighbors))
                # sum_res = 0.0
                # for j in range(num_neighbors - 1):
                #     neighbor = v.link_edges[j]
                #     same_faces = [f for f in neighbor.link_faces if f in v.link_faces]
                #     angles = []
                #     print('num faces', len(same_faces))
                #     for f in same_faces:
                #         print('num corners', len(f.loops))
                ...
                #     lap_bel_matrix[0, j] = res
                #     lap_bel_matrix[j, 0] = res
                #
                # lap_bel_matrix[0, 0] = sum_res
                #
                # # Solve for new vertex position.
                # mat = identity + FLOW_COEFFICIENT * dt * np.linalg.inv(lumped_mass_matrix) * lap_bel_matrix
                # x_new = np.linalg.inv(mat[:3, :3]) * x_old
                # print(x_new)
                # x_new = mathutils.Vector((x_new[0], x_new[1], x_new[2]))
                # vel = vel_old + (x_new - x_old) / dt
                ...
                if detected:
                    ...
                    for f in v.link_faces:
                        ...
                        for v_other in f.verts:
                            _, _, _, _, other_collided = keyframe_collisions[i - 1][v_other]
                            if other_collided:
                                num_collided += 1
