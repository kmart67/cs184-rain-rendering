...
def initscene():
    #floor
    addplane(location=(0,0,.3), rotation=(math.radians(-5), math.radians(60), 0))
    ...
    #adding camera
    bpy.ops.object.camera_add(location=(0,3,0.5), rotation=(math.radians(90), 0, math.radians(180)))
    ...
    # add droplet
    addsphere(location=(0,0,.8), size=0.2)
    name = "Droplet1"
    bpy.context.active_object.name = name
    ...
    """
    # sphere #2
    addsphere(location=(-.2,0,0.6), size=0.2)
    name = "Droplet2"
    bpy.context.active_object.name = name
    bpy.ops.object.shade_smooth()
    bpy.context.scene.objects.active = bpy.data.objects[name]
    ...
    layer_dict1 = add_new_attributes(droplet1)
    #layer_dict2 = add_new_attributes(droplet2)
    ...
    #meshmerge([droplet1, droplet2], [layer_dict1, layer_dict2])
    
    #fall_and_collide([droplet1, droplet2], bpy.context.scene.objects['Floor'], [layer_dict1, layer_dict2])
        
    fall_and_collide([droplet1], bpy.context.scene.objects['Floor'], [layer_dict1])
        
def meshmerge(droplets, layer_dicts):
    ...
    bpy.context.scene.objects.active = drop0
    bpy.ops.object.mode_set(mode='EDIT')
    ...
    bm = bmesh.new()
    bm.from_mesh( mesh0 ) # Add first mesh
    bm.from_mesh( mesh1 ) # Add 2nd mesh
    
    bm0 = bmesh.from_edit_mesh(mesh0)
    #bm1 = bmesh.from_edit_mesh(mesh1)
    
    #collided = layer_dicts[index]['collided']
    #mesh1 = layer_dicts[1]['mesh']
    #bm1 = layer_dicts[1]['bmesh']
    ...
    #bm = join_bmesh(bm1, bm0)
    bmesh.ops.automerge(bm, verts=bm0verts, dist=.1)
    bm.to_mesh(mesh0)
    bmesh.update_edit_mesh(mesh0, True)
...
"""
def fall_and_collide(droplets, plane, layer_dicts):
    ...
    for index in range(len(droplets)):
        ...
        mesh = layer_dicts[index]['mesh']
        bm = layer_dicts[index]['bmesh']
        collided = layer_dicts[index]['collided']
        print("droplet ", index)
